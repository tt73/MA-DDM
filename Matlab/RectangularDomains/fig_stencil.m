...
[Points,Interior,Boundary,NMatSDD1,CMatSDD,theta1] = buildMesh_Rect(x0,x1,y0,y1,h,1);
...
% 2 
[Points,Interior,Boundary,NMatSDD1,CMatSDD,theta1] = buildMesh_Rect(x0,x1,y0,y1,h,2);
ind = 5*N + 3; row = NMatSDD1(ind,:);
for i = 1:length(theta1)
   pts = Points( row((1:3)+(i-1)*3),:);
   plot(pts(:,1),pts(:,2),'k','LineWidth',1.5)
   scatter(pts(:,1),pts(:,2),20,'k','filled')
