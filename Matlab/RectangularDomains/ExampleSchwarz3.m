...
for k = 1:max_iter
   ...
   % solve
   if (k>1)
      [out1,~,newt1] = quadSolver3(Dom1.NMatLoc,CMatSDD(Dom1.Interior,:),Dvvs1,F1,uBdry1,epsilon,weight,h,u1(1:length(Dom1.Interior))); % call solver
      [out2,~,newt2] = quadSolver3(Dom2.NMatLoc,CMatSDD(Dom2.Interior,:),Dvvs2,F2,uBdry2,epsilon,weight,h,u2(1:length(Dom2.Interior))); % call solver
      newt_iters(k) = newt1+newt2;
   else
      [out1,~,newt1] = quadSolver3(Dom1.NMatLoc,CMatSDD(Dom1.Interior,:),Dvvs1,F1,uBdry1,epsilon,weight,h);
      [out2,~,newt2] = quadSolver3(Dom2.NMatLoc,CMatSDD(Dom2.Interior,:),Dvvs2,F2,uBdry2,epsilon,weight,h);
      newt_iters(k) = newt1+newt2;
   end
