function [Errors,Iterations,Times,Solutions,Domains,Parameters] = SchwarzSolver(N,overlap,choice,method,tol)
...
dirtime = tic;
[uSoln, ~,direct_iters] = quadSolver2(NMatSDD,CMatSDD,Dvvs,F,uBdry,epsilon,weight,h);

direct_time = toc(dirtime);
...
for k = 1:max_iter
   ...
   if method == 1
      [out1,~,newt1] = quadSolver2(Dom1.NMatLoc,Dom1.CMatLoc,Dvvs1,F1,uBdry1,epsilon,weight,h); % call solver
      [out2,~,newt2] = quadSolver2(Dom2.NMatLoc,Dom2.CMatLoc,Dvvs2,F2,uBdry2,epsilon,weight,h); % call solver
      newt_iters(k) = newt1+newt2;
   elseif method == 2
      [out1,~,newt1] = quadSolver2(Dom1.NMatLoc,Dom1.CMatLoc,Dvvs1,F1,uBdry1,epsilon,weight,h,u1(1:length(Dom1.Interior))); % call solver
      [out2,~,newt2] = quadSolver2(Dom2.NMatLoc,Dom2.CMatLoc,Dvvs2,F2,uBdry2,epsilon,weight,h,u2(1:length(Dom2.Interior))); % call solver
      newt_iters(k) = newt1+newt2;
   elseif method == 3
   ...
   u1(Dom1.ai) = (1-alpha)*u2(Dom2.ai)+alpha*u1(Dom1.ai);
   u2(Dom2.ai) = (1-alpha)*u1(Dom1.ai)+alpha*u2(Dom2.ai);
...
ddm_time = toc(ddmtime);

Errors = {ress,err_exact, err_direct, min_err };
Iterations = {conv_iter,newt_iters,direct_iters};
Times = {ddm_time, direct_time};
Solutions = {u_global, uSoln, exact, F, uBdry};
Domains = {Dom1,Dom2,Points,Interior};
Parameters = {h,depth,tol};
