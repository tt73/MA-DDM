...
do
   ...
   printf "now running [mpiexec -np $Nd ../../maddm -nks -N $N -problem ex5 -sub_snes_rtol $ntol -sub_ksp_rtol $ktol -op 0 -xmin -$limit -xmax $limit -ymin -$limit -ymax $limit -snes_converged_reason -snes_rtol $gtol]\n" >> $file.out
   mpiexec -np $Nd ../../maddm -nks -N $N -problem ex5 -sub_snes_rtol $ntol -sub_ksp_rtol $ktol -op 0 -xmin -$limit -xmax $limit -ymin -$limit -ymax $limit -snes_converged_reason -snes_rtol $gtol >> $file.out
done